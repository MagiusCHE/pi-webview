...
- `pnpm release -- --version 0.1.1` — prepara soltanto: bump versioni in entrambi i package.json, spostamento delle note inglesi da `CHANGELOG.md` / `[Unreleased]`, rebuild VSIX+bundle+UI, crea un tarball hashato e registra il manifest degli artefatti (gitignored). Non pubblica mai. Dopo il commit della preparazione, `pnpm release -- --publish [--tag <dist-tag>]` rifà la build completa, richiede worktree pulito, gestisce login/2FA npm via browser quando necessari, pubblica il medesimo tarball e ne verifica l'integrità sul registry npm e solo allora crea/verifica tag git e GitHub release. `--version --publish` è deliberatamente rifiutato: un tag non deve mai puntare a un commit precedente al bump. `pnpm release -- --publish --publish-only` è un retry idempotente che richiede manifest, commit e hash degli artefatti identici alla build completa già riuscita.
- **Release — build completa obbligatoria**: quando l'utente chiede una nuova release, compilare sempre tutti gli artefatti sulla macchina corrente, inclusi companion Chrome e VSIX Visual Studio tramite Wine quando si opera su Linux. Non riutilizzare artefatti preesistenti o obsoleti e non chiedere se si debba compilare tutto: la richiesta di release autorizza e richiede la build completa. `--publish-only` è ammesso esclusivamente come retry della fase di pubblicazione dopo che la stessa release e la stessa versione hanno già completato con successo la build completa; non sostituisce mai la build iniziale.
- **Version bump — mai manuale**: cambiare la versione esclusivamente tramite `pnpm release -- --version <version>`, poi fare commit e usare `pnpm release -- --publish`; non modificare mai a mano i campi `version` nei `package.json`. `--version` e `--publish` non sono combinabili. La preparazione sposta automaticamente le note inglesi da `CHANGELOG.md` / `[Unreleased]` alla nuova versione; una release senza note deve fallire.
