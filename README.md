...
- **IDE and browser integration**: the same UI in **VS Code**, **Visual Studio 2022/2026** and a **Chrome Side Panel**. Chrome shows the active page’s favicon/title/URL and selection, can atomically move an existing standalone session into the panel, and exposes consent-gated complete/targeted DOM, viewport-screenshot, visual/coordinate click, CSS mutation, navigation, scrolling and structured page-action tools to the agent
...
| `pnpm release -- --version <x.y.z>`            | Preparation only: updates both package versions, moves English `[Unreleased]` notes, rebuilds every artifact, creates a hashed npm tarball, and records artifact hashes. It never publishes.                                                                                                       |
| `pnpm release -- --publish [--tag <dist-tag>]` | From the committed preparation, rebuilds every artifact, requires a clean worktree, completes npm browser authentication if needed, publishes that exact tarball and verifies its registry integrity, then pushes tag `v<x.y.z>` and creates a GitHub release with the same hash-verified archive. |
| `pnpm release -- --publish --publish-only`     | Idempotent recovery only after a matching complete build; it requires the recorded manifest, commit and artifact hashes before retrying npm publication, tag and GitHub release.                                                                                                                   |

`--version` and `--publish` are intentionally mutually exclusive, so a release tag
cannot point to a commit that predates its version bump.
