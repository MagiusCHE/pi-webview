...
- **Sessions** — switch, rename, delete, filter by folder, move or fork across workspaces and create new sessions. Changing the folder from the session picker offers to move the current session there (the conversation continues in the new folder and the old copy is removed), to fork it (a copy, the original stays) or to start a new session. Confirmed session-changing operations immediately show the loading overlay and lock the complete UI until the refreshed history is ready. Resume summaries include relative activity, compaction count and session-file size. Browser refresh resumes the same session in its saved workspace
